...
function expectedPaydays(count = 12){
  ...
  let current = dates.at(-1) || new Date(state.profile.joiningDate + "T12:00:00");
  const result = [];
...
function renderDashboard(){
  ...
  $("dashboardCompany").textContent = profile.company;
  $("joiningText").textContent = new Date(`${profile.joiningDate}T12:00:00`).toLocaleDateString("en-US", {month: "long", day: "numeric", year: "numeric"});
...
function saveProfile(){
  state.profile.name = $("profileName").value.trim() || "Kumari Pushpa Rani";
  state.profile.company = $("profileCompany").value.trim() || "Natera";
  state.profile.joiningDate = $("profileJoining").value || "2026-03-16";
...
async function enableNotifications(){
  ...
  if(permission === "granted"){
    new Notification("Pushpa Payroll Hub", {
...
function sendDueNotification(){
  ...
  const today = new Date().toISOString().slice(0, 10);
  const notificationKey = "pushpaPayrollHubV5LastNotice";
...
function backupPayroll(){
  ...
  persist();
  downloadBlob(JSON.stringify({version: APP_VERSION, state}, null, 2), "Pushpa_Payroll_Backup.json", "application/json");
...
async function backupFull(){
  ...
  downloadBlob(
    ...
    "Pushpa_Payroll_Full_Backup.json",
    "application/json"
  );
...
function setupServiceWorker(){
  ...
  navigator.serviceWorker.register("sw.js?v=5.0.0").then(registration => {
    registration.update();
